...
#undef max
	...
	template <typename T, const int N, const int M, const int L>
	static inline vecN<T, L> operator*(const matNM<T, N, M>& mat, const vecN<T, L>& vec)
	{
		vecN<T, N> vecR = convert<T, L, N>(vec);
		...
		for (m = 0; m < M; m++)
		{
			for (n = 0; n < N; n++)
			{
				result[m] += vecR[n] * mat[n][m];
			}
		}

		vecN<T, L> resultR = convert<T, M, L>(result);

		return resultR;
	}

	template <typename T, const int N1, const int N2>
	static inline vecN<T, N2> convert(vecN<T, N1> vec)
	{
		vecN<T, N2> result(T(0));
		int minN = min(N1, N2);
